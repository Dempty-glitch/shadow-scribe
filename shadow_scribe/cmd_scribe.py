...
def cmd_scribe(mock: bool) -> None:
    ...
    if mock:
        print("🧪 MODE: DRY-RUN (mock data, no file writes)\n")
        brief_path = RAW_LOGS_DIR / "session_brief_mock.md"
        diff_path = RAW_LOGS_DIR / "git_diff_mock.txt"
    else:
        # ── Phase 3.3: Directory-based Routing ──────────────────────────────
        # Priority: scan raw_logs/ for subdirectory containing session_brief.md
        # Fallback: read flat file (backward compat with v1.1.0)
        ...
        if project_subdirs:
            for d in project_subdirs:
                if (d / "session_brief.md").exists():
                    ...
                    break
            if not project_dir:
                project_dir = project_subdirs[0]  # Fallback: most recent directory
        ...
        if project_dir:
            ...
            print("⚠️  No project subdirectory found. Fallback: flat raw_logs/ (v1.1.0 compat)")
    ...
    # 1.5. Extract Plan Path and Load Plan (Tier 2 Audit)
    plan_content = ""
    ...
    if plan_match:
        ...
        if (
            ...
            if plan_path.exists() and plan_path.is_file():
                ...
                print(f"⚠️  Plan Path declared as '{plan_path_str}' but file does not exist. Skipping Plan.")

    print()

    # 2. Call Gemini
    raw_output = call_gemini(brief, diff, plan_content)
    print(f"✅ Gemini returned {len(raw_output)} chars\n")
    ...
    print("─" * 60)
    print("📄 PART 1: Full Session Log")
    print("─" * 60)
    print(session_log)
    print()
    print("─" * 60)
    print("📋 PART 2: Index Row (1 line to insert into INDEX_MATRIX)")
    print("─" * 60)
    print(index_row)
    print()
    ...
    if mock:
        ...
        print("If output looks good → rerun WITHOUT --mock to save for real.")
        print("═" * 60)
    else:
        # Get date from brief
        ...
        # ── Phase 3.3: Soft-Delete → Trash (no permanent deletion) ────────────
        ...
        # ── Auto-cleanup: Remove trash older than 30 days ────────────────────────
        retention_days = 30
        ...
        cleanup_msg = f" (cleaned {deleted_count} trash folder(s) > 30 days)" if deleted_count > 0 else ""
        print(f"✅ Index updated | 🗑️ raw_logs → trash/{cleanup_msg}")
        ...
        if risks_match:
            ...
            if has_real_risk:
                ...
                print("🔍 Audit Pass: No 🔴/🟡 risks detected")
        print("═" * 60)
